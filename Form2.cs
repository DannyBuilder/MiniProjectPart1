...
namespace MiniProjectPart1
{
    public partial class Form2 : Form
    {
        ...
        private void PopulateCountriesComboBox()
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(@"Data Source=LAB108PC18\SQLEXPRESS;Initial Catalog=Tourism;Integrated Security=True"))
                {
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(@"Data Source=LAB108PC18\SQLEXPRESS;Initial Catalog=Tourism;Integrated Security=True"))
                {
                    ...
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        ...
                        while (reader.Read())
                        {
                            comboBoxTowns.Items.Add(reader["Town_Name"].ToString());
                        }
                        reader.Close();
                    }
                }
        ...
        private void comboBoxTowns_SelectedIndexChanged(object sender, EventArgs e)
        {
